        ...
        for p in policy_list:
            ...
            try:
                Policy.objects.create(
                    ...
                    bizPrdBgngYmd = to_int_or_none(p.get('bizPrdBgngYmd')),
                    bizPrdEndYmd = to_int_or_none(p.get('bizPrdEndYmd')),
                    bizPrdEtcCn = to_int_or_none(p.get('bizPrdEtcCn')),
                    ...
                    sprtTrgtAgeLmtYn = to_int_or_none(p.get('sprtTrgtAgeLmtYn')),
                    mrgSttsCd = p.get('mrgSttsCd', ''),
                    earnCndSeCd = p.get('earnCndSeCd', ''),
                    earnMinAmt = to_int_or_none(p.get('earnMinAmt')),
                    arnMaxAmt = to_int_or_none(p.get('arnMaxAmt')),
                    earnEtcCn = p.get('earnEtcCn', ''),
                    addAplyQlfcCndCn = p.get('addAplyQlfcCndCn', ''),
                    ptcpPrpTrgtCn = p.get('ptcpPrpTrgtCn', ''),
                    nqCnt = to_int_or_none(p.get('nqCnt')),
                    rgtrInstCdNm = p.get('rgtrInstCdNm', ''),
                    rgtrHghrkInstCdNm = p.get('rgtrHghrkInstCdNm', ''),
                    zipCd = p.get('zipCd', ''),
                    ...
                    lastMdfcnDt = p.get('lastMdfcnDt', ''),
                    sBizCd = to_int_or_none(p.get('sBizCd')),
                )
                saved += 1
            except Exception as e:
